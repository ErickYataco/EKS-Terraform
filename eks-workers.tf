data "aws_ami" "eks-worker" {
  filter {
    name   = "name"
    values = ["amazon-eks-node-${aws_eks_cluster.torus.version}-v*"]
  }
...
resource "aws_launch_configuration" "demo" {
  ...
  instance_type                 = var.node-istance-type
  name_prefix                   = "node-${var.project}-${var.env}"
